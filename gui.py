...
class AssistantGUI:
    ...
    def _build_top(self) -> None:
        ...
        # metrics
        self.metrics_label = ctk.CTkLabel(top, text="Model: – | 0 tokens | 0.0 s", font=("Arial", 11))
        ...
        # orb canvas
        self.wave_canvas = tk.Canvas(self.root, width=460, height=90, bg="#2b2b2b", highlightthickness=0)
        ...
        # context strip
        self.context_label = ctk.CTkLabel(self.root, text="Context: –", font=("Arial", 12), anchor="w")
    ...
    def _smooth_orb(self) -> None:
        ...
        try:
            ...
            if self.wave_canvas and self.wave_canvas.winfo_exists():
                self.wave_canvas.coords(self.orb, cx - radius, cy - radius, cx + radius, cy + radius)
                self.wave_canvas.delete("wave_line")
                if self._target_amplitude > 0.05:
                    h = int(20 * self._orb_amplitude)
                    self.wave_canvas.create_line(20, 85 - h, 440, 85 - h, fill="#00ffbf", width=2, tag="wave_line")
