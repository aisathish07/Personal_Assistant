...
class TextToSpeech:
    ...
    # ------------------------------------------------------------------
    async def speak(self, text: str, status_callback: Optional[Callable[[str, Any], None]] = None) -> None:
    ...
    async def _generate_audio(self, text: str) -> Optional[str]:
        ...
        for engine in Config.TTS_PRIORITY:
            try:
                if engine == "elevenlabs" and Config.ELEVENLABS_API_KEY and ELEVEN_SDK_NEW is not None:
        ...
        def _piper():
            try:
                ...
                proc = subprocess.Popen(
                    [self.piper_exe, "--model", "en_US-amy-medium.onnx",
