...
# ------------------------------------------------------------------
try:
    from llm_manager import get_llm_response, close_sessions
except ImportError:
...
# ------------------------------------------------------------------
from web_agent import WebAgent               # was missing → web features dead
from app_controller import AppController     # was missing → in-app controls dead
...
def ensure_certifi() -> None:
    try:
        ...
        for action in (["uninstall", "-y", "certifi"],
...
class CommandProcessor:
    def __init__(
        ...
        # metrics
        self.metrics: Dict[str, Any] = {"total_tokens": 0, "last_latency": 0.0, "last_provider": None}
        ...
        self.intents = {
            ...
            "app_command": ["in chrome", "in spotify", "in discord", "in vscode", "in notepad", "in whatsapp"],
        }
    ...
    async def _load_and_schedule_reminders(self) -> None:
        ...
        for r in reminders:
            ...
            if self.scheduler:
                try:
                    self.scheduler.add_job(self._trigger_reminder, "date", run_date=run_date, args=[r["id"], r["text"]])
                    count += 1
    ...
    async def execute(self, command_text: str) -> str:
        ...
        try:
            if intent == "remember_preference":
                ...
                response_text = await asyncio.to_thread(self.open_app, m.group(1)) if m else "Which application?"
    ...
    async def web_search(self, text: str) -> str:
        ...
        results = await self.web_agent.google_search(query, num_results=3)
        if not results:
            return "I couldn't get any results."
        reply = "Top results: " + " | ".join(r["title"] for r in results)
        return reply
        ...
        if args:
            ...
            for bit in args.split():
                if "=" in bit:
                    k, v = bit.split("=", 1)
                    params[k] = v
        return await asyncio.to_thread(self.app_controller.execute_command, app.lower(), cmd.lower(), **params)
    ...
    async def handle_unknown(self, text: str) -> None:
        ...
        try:
            ...
            raw = await asyncio.wait_for(
                get_llm_response(prompt, provider=provider),
                timeout=30
            )
    ...
    async def set_reminder(self, text: str) -> str:
        ...
        if self.scheduler:
            try:
                self.scheduler.add_job(self._trigger_reminder, "date", run_date=run_date, args=[rid, rem_text])
            except Exception as e:
                logger.error("Failed to schedule reminder: %s", e)
